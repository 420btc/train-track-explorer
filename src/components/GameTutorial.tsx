...
const GameTutorial: React.FC<GameTutorialProps> = ({ 
  ...
  // Validar que currentStep esté dentro del rango válido
  const safeCurrentStep = Math.max(0, Math.min(currentStep, tutorialSteps.length - 1));
  
  // Obtener el paso actual de forma segura
  const getCurrentStep = (): TutorialStep => {
    if (tutorialSteps.length === 0) {
      return {
        title: "Tutorial",
        content: "Cargando tutorial...",
        icon: <Train className="h-12 w-12 text-primary" />,
        highlight: undefined
      };
    }
    return tutorialSteps[safeCurrentStep] || tutorialSteps[0];
  };
            ...
            {currentTutorialStep.highlight && (
              <div className="mt-2 p-3 bg-primary/10 border border-primary/20 rounded-lg w-full">
                <p className="text-sm font-medium text-primary">
                  {currentTutorialStep.highlight === 'basics' && '¡Consejo! Haz clic en una vía para mover tu tren.'}
                  {currentTutorialStep.highlight === 'map' && '¡Consejo! Usa el ratón para moverte por el mapa y la rueda para hacer zoom.'}
                  {currentTutorialStep.highlight === 'passengers' && '¡Consejo! Los pasajeros aparecen cada cierto tiempo en las estaciones.'}
                  {currentTutorialStep.highlight === 'train' && '¡Consejo! Tu tren tiene una capacidad limitada de pasajeros.'}
                  {currentTutorialStep.highlight === 'resources' && '¡Consejo! Mantén la felicidad alta para atraer más pasajeros.'}
                  {currentTutorialStep.highlight === 'start' && '¡Consejo! Puedes volver a ver este tutorial desde el menú lateral.'}
                </p>
              </div>
            )}
